function out = ImageDxx(in, varargin)
%% Computes the second image derivative in x-direction.
%
% out = ImageDxx(in, varargin)
%
...
%            'standard-4' = standard 4th order scheme [-1 16 -30 16 -1]/12.
...
% Output parameters
%
% out : Second Image x-derivative of input image in.
%
% Description:
%
% The function computes the second derivative in x-direction of an image (the
% origin being in the top left corner) using a finite difference scheme.
%
% Example:
%
% I = rand(256,256);
% Ixx = ImageDxx(I);
%
% See also ImageDx, ImageDy, ImageDyy, ImageDxy, filter2, imfilter.
...
% Last revision on: 07.05.2012 22:06
...
parser.addParamValue('scheme', 'standard', @(x) strcmpi(x, validatestring( x, ...
    {'standard', 'standard-4', 'separated', 'user-specified'}, ...
    mfilename,'scheme') ) );
...
% Use a convolution to compute the image derivatives.
switch opts.scheme
    
    case 'standard'
        out = filter2( [0 0 0 ; 1 -2 1 ; 0 0 0]/hx^2, ...
            out, 'valid' );
    
    case 'standard-4'
        out = filter2( [0 0 0 0 0 ; -1 16 -30 16 -1 ; 0 0 0 0 0]/(12*hx^2), ...
            out, 'valid' );

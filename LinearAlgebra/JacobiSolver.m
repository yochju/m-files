function [x, varargout] = JacobiSolver(A, b, varargin)
%% Jacobi solver for linear systems.
%
% [x, varargout] = JacobiSolver(A, b, varargin)
%
...
% A = T'*T + 200*eye(3);
% b = randi(10,[3,1]);
% [x1 x2 x3 x4 x5] = JacobiSolver(A,[b 2*b.^2],'maxit',10000)
%
% See also GaussSeidelSolver
...
% Last revision on: 18.05.2013 19:56
...
while ((it <= opts.maxit) && (max(relres) > opts.tol))
    x = (1-opts.relax)*x + opts.relax*(b - UL*x)./kron(ones(1,size(b,2)),D);
    if nargout >= 3
        if ~isempty(opts.M2)

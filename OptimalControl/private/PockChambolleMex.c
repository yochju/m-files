        ...
        )
{
    ...
    for ( ii = 0; ii < L; ii++ )
    {
        ...
        for ( jj = 0; jj < Anrow; jj++ )
        {
            ...
            /* Update c:
             * c = shrink( (c - tau*B*y + tau*mu*cb)/(1+tau*eps+tau*mu) , tau*lambda/(1+tau*eps+tau*mu) );
             */
            c[jj] =         sgn( c[jj] - tau*b[jj]*y[jj] + tau*mu*cb[jj] ) *
                    max0( (fabs( c[jj] - tau*b[jj]*y[jj] + tau*mu*cb[jj] ) - tau*lambda) * dummy);

function [u, c] = OCDenoise(f, lambda, varargin)
...
while i <= opts.maxit
    ...
    % A = C - (I-C)*D;
    A = I - spdiags(ToVec(cbar), 0, N, N) - spdiags(ToVec(cbar), 0, N, N)*D;
    ...
    tic();
    [utemp c j du dc] = PockChambolleL2Mex( ToVec(f), ToVec(cbar), A, A', bb, g, opts.e, opts.mu, opts.lambda, opts.PockIt, opts.PockTol);

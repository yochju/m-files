function [ Err ] = ErrorInterpInf(f, x)
% [ Err ] = ErrorInterpInf( f, x )
% ErrorInterpInfLoc computes the global L-infinity interpolation error between
% two consecutive knots for the function f and its piecewise linear spline
% interpolation with knots x.
%
% Usage: [ Err ] = ErrorInterpInf( f, x )
...
ErrLoc = FreeKnot.ErrorInterpInfLoc(f, x);
Err = max(ErrLoc(:));

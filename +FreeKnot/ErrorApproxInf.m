function [ Err ] = ErrorApproxInf(f, x)
% [ Err ] = ErrorInterpInf( f, x )
% ErrorApproxInfLoc computes the global L-infinity approximation error between
% two consecutive knots for the function f and its piecewise linear spline
% interpolation with knots x.
%
% Usage: [ Err ] = ErrorApproxInf( f, x )
...
ErrLoc = FreeKnot.ErrorApproxInfLoc(f, x);
Err = max(ErrLoc(:));
end
